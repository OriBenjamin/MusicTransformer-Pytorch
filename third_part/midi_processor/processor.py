...
RANGE_VEL = 32
RANGE_HAND_LOC = 52
SIZE_OF_FIVE_KEYS = 112.4 # mm
SIZE_OF_KEY = SIZE_OF_FIVE_KEYS/5 # mm
START_IDX = {
    'note_on': 0,
    'note_off': RANGE_NOTE_ON,
    'time_shift': RANGE_NOTE_ON + RANGE_NOTE_OFF,
    'velocity': RANGE_NOTE_ON + RANGE_NOTE_OFF + RANGE_TIME_SHIFT,
    'hand_loc': RANGE_NOTE_ON + RANGE_NOTE_OFF + RANGE_TIME_SHIFT + RANGE_VEL
}  
...
class Event:
    ...
    def _type_check(int_value):
        ...
        if int_value in range_note_on:
            ...
            return {'type': 'hand_loc', 'value': valid_value}
...
def _event_seq2snote_seq(event_sequence):
    # gets rid of "time_shift" and "velocity" events and instead embeds them in the "note_on" and "note_off" events
    timeline = 0
    velocity = 0
    snote_seq = []

    for event in event_sequence:
        if event.type == 'hand_loc': ## hand_loc is not used inside midi
            continue
...
import pandas as pd
def get_hand_position_from_time(df, time):
    # search the first row where the velocity is not 0
    vel_ind = 0
    for i in range(len(df['velocity'])):
        if df['velocity'].iloc[i] > 0:
            vel_ind = i
            break
    vel_time = df['time'].iloc[vel_ind]
    real_time = time - 1 + vel_time 
    thumb_pos = df['thumb4'].iloc[len(df)-1][1]
    for i in range(len(df)):
        if df['time'].iloc[i] > real_time:
            thumb_pos = df['thumb4'].iloc[i-1][1]
            break
    device_pos = df['dev_pos'].iloc[0][1]
    key_pos = 26 + int((device_pos-thumb_pos)/SIZE_OF_KEY)*(1 if thumb_pos>device_pos else -1) -1
    assert key_pos >= 0 and key_pos < 52
    return Event(event_type='hand_loc', value=key_pos)
...
def encode_midi(file_path):
    ...
    for inst in mid.instruments:
        ...
        # add one sustain for the entire track
        ctrls.append(SustainDownManager(0, inst.get_end_time()))

        notes += _note_preprocess(ctrls, inst_notes)
        # print()
        # print("notes: ", notes, "file_path: ", file_path, "mid:", mid, "inst:", inst, "ctrls:", ctrls, "inst_notes:")
    ...
    cur_vel = 0
    ####
    new_file_path = "dataset_with_hands/h5s/" + file_path.split('/')[-1].split('.')[0] + ".h5"
    print("using h5 file: ", new_file_path, "for mid file: ", file_path)
    df = pd.read_hdf(path_or_buf=new_file_path, key='df')
    for snote in dnotes:
        # print(cur_time, snote.time)
        if cur_time >= 1:
            events += [get_hand_position_from_time(df, cur_time)]
        events += _make_time_sift_events(prev_time=cur_time, post_time=snote.time)
        # add an event for hand position at the time of the note (which is cur_time + real start time)
    ...
    #         events.(Event(event_type='hand_loc', value=mid.thumb3))
