...
## Overview
This project explores how to incorporate hand position data into MIDI-based music generation using a Transformer model. A primary challenge was representing both MIDI and hand position data in a structured way that could be effectively processed by the model.

### Data Representation
- **MIDI Events**: Encoded as tokens representing musical actions (e.g., note-on, note-off, time-shifts).

- **Hand Position Tokens**: Represented as x-coordinates of the hand's positional shifts. We defined a token range specifically for hand positions - the x-coordinates were mapped using RANGE_HAND_LOC = 52, with each key's width derived from SIZE_OF_FIVE_KEYS = 112.4 mm (measured through the keyboard size the data was taken from, and can be adjusted accordingly).
Initially, we considered integrating the hand position data directly into note tokens. However, this approach would have completely altered the token structure, requiring additional modifications to datasets without hand data and complicating the reuse of pre-trained models. Instead, we opted for a separate token representation, which allows for easy removal or inclusion of hand data without affecting the core token structure.

- **Dataset Alignment and Normalization**: The MIDI and hand position data came from different sources, requiring alignment due to discrepancies in instrument size. A key adjustment involved shifting hand position data to match MIDI note positions accurately. We found that hand positions recorded in the h5 files were consistently offset by approximately 20 units from the MIDI note positions due to differences in instrument size. After confirming this offset, we systematically corrected the alignment, ensuring that the hand position data accurately matched the corresponding MIDI events. We then normalized the hand positions using the keyboard size measurements to align with note placements.
